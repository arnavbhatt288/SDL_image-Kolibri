...
static SDL_bool GetStringBoolean(const char *value, SDL_bool default_value) {
  if (!value || !*value) {
...
static const Format formats[] = {
    ...
    },
    {
        ...
        100,
        IMG_INIT_JPG,
#if (USING_IMAGEIO && defined(JPG_USES_IMAGEIO)) ||                            \
    defined(SDL_IMAGE_USE_WIC_BACKEND) || defined(LOAD_JPG)
        SDL_TRUE,
#else
        SDL_FALSE,
#endif
    ...
    },
    {
        ...
        0, /* lossless */
        IMG_INIT_PNG,
#if (USING_IMAGEIO && defined(PNG_USES_IMAGEIO)) ||                            \
    defined(SDL_IMAGE_USE_WIC_BACKEND) || defined(LOAD_PNG)
        SDL_TRUE,
#else
        SDL_FALSE,
#endif
...
static SDL_bool StrHasSuffix(const char *str, const char *suffix) {
...
static SDL_bool ConvertToRgba32(SDL_Surface **surface_p) {
  if ((*surface_p)->format->format != SDL_PIXELFORMAT_RGBA32) {
    SDL_Surface *temp;

    temp = SDL_ConvertSurfaceFormat(*surface_p, SDL_PIXELFORMAT_RGBA32, 0);
    SDLTest_AssertCheck(temp != NULL, "Converting to RGBA should succeed (%s)",
                        SDL_GetError());
    if (temp == NULL) {
      return SDL_FALSE;
    }
...
static void FormatLoadTest(const Format *format, LoadMode mode) {
  ...
  if (mode != LOAD_CONVENIENCE) {
    ...
    SDLTest_AssertCheck(src != NULL, "Opening %s should succeed (%s)", filename,
                        SDL_GetError());
    if (src == NULL)
      goto out;
  }
  ...
  case LOAD_RW:
    if (format->checkFunction != NULL) {
      ...
      SDLTest_AssertCheck(ref_src != NULL, "Opening %s should succeed (%s)",
                          refFilename, SDL_GetError());
      if (ref_src != NULL) {
        check = format->checkFunction(ref_src);
        SDLTest_AssertCheck(!check, "Should not detect %s as %s -> %d",
    ...
    if (format->checkFunction != NULL) {
      ...
      SDLTest_AssertCheck(check, "Should detect %s as %s -> %d", filename,
                          format->name, check);
    }
  ...
  if (!SDLTest_AssertCheck(surface != NULL, "Load %s (%s)", filename,
                           SDL_GetError())) {
    goto out;
  }

  SDLTest_AssertCheck(surface->w == format->w, "Expected width %d px, got %d",
                      format->w, surface->w);
  SDLTest_AssertCheck(surface->h == format->h, "Expected height %d px, got %d",
                      format->h, surface->h);
  ...
  if (reference != NULL) {
    ...
    SDLTest_AssertCheck(
        diff == 0, "Surface differed from reference by at most %d in %d pixels",
        format->tolerance, diff);
    if (diff != 0 ||
        GetStringBoolean(SDL_getenv("SDL_IMAGE_TEST_DEBUG"), SDL_FALSE)) {
      DumpPixels(filename, surface);
...
static void FormatSaveTest(const Format *format, SDL_bool rw) {
  ...
  SDL_snprintf(filename, sizeof(filename), "save%s.%s", rw ? "Rwops" : "",
               format->name);
  ...
  if (format->canLoad) {
    ...
    if (!SDLTest_AssertCheck(surface != NULL, "Load %s (%s)", "saved file",
                             SDL_GetError())) {
      goto out;
    }
    ...
    SDLTest_AssertCheck(surface->h == format->h,
                        "Expected height %d px, got %d", format->h, surface->h);
...
static void FormatTest(const Format *format) {
  SDL_bool forced;
  char envVar[64] = {0};
  ...
  if (forced) {
    SDLTest_AssertCheck(format->canLoad, "%s loading should be enabled",
                        format->name);
  }
  ...
  if (format->canLoad || forced) {
    ...
    if (SDL_strcmp(format->name, "SVG-sized") == 0) {
      ...
      if (SDL_strcmp(format->name, "TGA") == 0) {
        SDLTest_Log("SKIP: Recognising %s by magic number is not supported",
                    format->name);
      } else {
  ...
  forced = GetStringBoolean(SDL_getenv(envVar), SDL_FALSE);
  if (forced) {
    SDLTest_AssertCheck(format->canSave, "%s saving should be enabled",
                        format->name);
  }
...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc;) {
    ...
    if (consumed < 0) {
      ...
      static const char *options[] = {"[--iterations #]", "[--execKey #]",
                                      "[--seed string]",
                                      "[--filter suite_name|test_name]", NULL};
  ...
  /* Call Harness */
  result = SDLTest_RunSuites(testSuites, (const char *)userRunSeed, userExecKey,
                             (const char *)filter, testIterations);
